...
这个系列写到现在，前五篇分别聊了 Skill 系统、ReAct 循环、Context Engineering、文件系统记忆和 RAG 长期记忆。每一篇都是独立的概念加上能跑的代码片段，但要真正做成一个产品，中间还差不少东西——消息从哪来、会话怎么隔离、代码在哪执行、附件怎么处理。

这篇就来把这些缺的部分补上。最终产物是**小圈**，一个跑在飞书上的私人工作助手，把前面所有概念串进一个真实可用的项目里。
...
这几个问题分别对应了 Session 系统、记忆系统、沙箱执行、飞书接入，以及把它们串起来的 Runner。
...
文件记忆的接入点在 `runAgent` 入口——每次调用都从 `workspace/` 读取 `soul.md`、`user.md`、`agent.md`、`memory.md`，拼进系统提示词。Agent 运行中可以通过 `write_file` 修改这些文件，下次对话自动生效。

上下文管理跑在 ReAct 循环里，每一轮迭代都会执行。先调 `pruneToolResults` 裁剪早期工具返回，再检查 token 数，超过阈值就调 `maybeCompress` 压缩旧对话为摘要。压缩后的上下文存到 `data/ctx/{sessionId}_ctx.json`，下次用户发消息时直接恢复。

RAG 记忆的接入分写和读两个点。写在 Runner 里——每次 Agent 回复后，异步调用 `storeMemory` 把对话存入 PostgreSQL + pgvector：
...
读的一端是 `memory_search` 工具，按 `routingKey` 隔离，每个用户只能搜到自己的历史。这里有个设计取舍：我们没有每次都自动检索（那样浪费 token），而是把它当普通工具暴露给 Agent，让它自己判断什么时候需要翻历史。
...
小圈的模块划分：**Runner 管调度，Session 管状态，沙箱管执行，记忆管上下文**。每个模块的职责边界清楚，替换任何一个都不影响其他——把飞书换成微信，只需要重写 `feishu/` 目录，Runner 和 Agent 不用动。
