...
整个 memory-save 的逻辑写在一个 Skill 文件里，不是代码，是**结构化的自然语言指令**。先看完整内容：
...
## 2.3 Skill 文件规范 + skill-creator

skill-creator 本身也是一个 Skill，负责引导 Agent 把用户描述的 SOP 转化为标准的 Skill 文件。Anthropic 官方有一个 [skill-creator 插件](https://github.com/anthropics/claude-code-skill-creator)，从需求理解到打包分发都覆盖了。不过官方版面向 Claude Code 的 Skill 分发生态，对我们的 demo 太重。这里基于它的思路写一个简化版：
...
**explain the why。** 没有原因的 NEVER，模型在 edge case 会自行决定要不要遵守；说了原因，模型才能在没见过的情况下举一反三。所以 Skill 里每条规则都要解释为什么。
...
memory-governance 的逻辑同样写在 Skill 文件里，包含 8 类检查，每类都写了"检查什么"和"为什么"：
...
Demo 跑通了，来看看生产级的 Agent 怎么做。翻一下 Claude Code 的源码。
...
上一篇的数据流是单向的：文件 -> System Prompt -> 对话。这篇加了反向链路：对话 -> Skill 系统 -> 文件。Agent 能读也能写了。
...
但是文件系统只对数据量较小的场景有用，数据量大时就需要使用搜索了，这个就留作后面再讨论吧。
