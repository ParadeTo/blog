...
其中，`CONSTITUTION.md` 是项目的"戒律"，定义了所有开发者（包括 AI）必须遵守的铁律，确保代码简单、安全、可测试、高质量：

```
# Frontend Constitution (前端开发宪章)
...
## 第三条：类型安全与明确性原则 (Type Safety and Explicitness)
...
```

其中，`CLAUDE.md` 是一个"AI 说明书"，确保每次 Claude Code 帮你写代码时，都遵循项目的技术栈、规范和最佳实践：
...
# --- 核心使命与角色设定 ---
你是一个资深的前端工程师，你的所有行动都必须严格遵守上面导入的项目宪法（CONSTITUTION.md）。
...
```
@specs/001-core-functionality/tasks.md 请创建一个 agent team 完成剩余的 phase 开发
```
