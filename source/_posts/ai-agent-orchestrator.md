...
接下来我们用代码实现一个简化版的 Orchestrator，让它能协调多个子 Agent 完成一个软件开发任务。技术栈和这个系列前面几篇一样，TypeScript + Vercel AI SDK（`ai` 包）。核心思路也不复杂：Orchestrator 本身就是一个普通的 `generateText` 调用加上 tool-use，只不过它调用的 tool 里面会再发起新的 `generateText` 调用，每一个都是一个独立的子 Agent，拥有自己的上下文。
...
async function runSubAgent(params: SubAgentParams): Promise<string> {
  const { role, goal, task, context, tools, outputFile } = params
...
`tools` 也是外部传入的。哪些工具给哪个子 Agent 用，由主 Agent 在派发时决定。架构设计师可能只需要 `readFile` 和 `writeFile`，而测试工程师可能还需要 `runCommand` 来跑测试脚本。工具的分配本身就是权限控制的一部分。
...
```typescript
async function runOrchestrator(requirementPath: string) {
...
`maxSteps: 50` 比子 Agent 的 15 要大得多。这是因为 Orchestrator 的每一轮操作涉及多个 tool 调用：spawn 一个子 Agent 是一次调用，读取产出文件做验收又是一次，验收不通过再 spawn Debugger 又是一次。一个典型的流程（spawn 架构师 → 读产出 → spawn 接口设计师 → 读产出 → 并发 spawn 前后端 → 读产出 → 验收 → 可能重试）轻轻松松就要 20-30 步。
...
[并发启动] 2 个子 Agent 同时运行...
  ...
  [完成] → backend/server.js

--- 验收 ---
[主 Agent] 验收前端 ✓
[主 Agent] 验收后端 → 测试未通过 ✗
  → 分析：test_create_leave 断言失败，status 字段返回 null
  → 根因：create 接口未设置默认 status = "待审批"

[sub-agent: Debugger] 启动 (独立上下文)
  context: 失败报告 + 根因分析 + 修复建议
[sub-agent: Debugger] 完成 → backend/server.js (修复版)
[主 Agent] 验收后端 ✓
...
**串行阶段**（架构设计 → 接口定义 → Mock）：这三步有明确的前后依赖，架构方案决定了接口怎么定义，接口定义决定了 Mock 数据怎么造，所以必须一个接一个地跑。

**并发阶段**（前端 + 后端同时开发）：接口定义和 Mock 数据就位后，前端和后端的开发工作互不依赖。主 Agent 调用 `spawnParallel` 同时启动两个子 Agent，两边并行推进。

**验收 + 重试**：后端验收没通过，但主 Agent 没有简单地把同样的任务再发一遍。它先分析了失败原因（`test_create_leave` 断言失败，`status` 字段返回 `null`），定位了根因（`create` 接口没设置默认状态），然后 spawn 了一个 Debugger 子 Agent，在 context 里带上了失败报告、根因分析和修复建议。这是有针对性的修复，不是盲目重试。
...
第一个坑是**没有 kill switch**。你给 Orchestrator 设了 `maxSteps: 50`，子 Agent 设了 `maxSteps: 15`，看起来有上限了。但如果主 Agent 反复验收不通过、反复 spawn Debugger 重试呢？它不会觉得"我已经试了五次了该放弃了"，它会一直试下去直到 maxSteps 耗尽。HackerNews 上有人分享过案例，一个没设预算上限的 Agent 跑了一整晚，醒来发现 $600 账单换来零产出。解决办法很朴素：给 Orchestrator 加一个全局的 step 计数器，跑超了就强制停止，输出一份当前进度报告让人来接手。
