...
你写过搜索功能吗？用户输入关键词，后端查数据库，把结果返回给前端展示——这就是最朴素的 RAG。传统搜索和 AI 时代的 RAG 结构完全一样，就三步：

1. **建索引（Indexing）**：把知识库预处理，建立可检索的结构
2. **检索（Retrieval）**：拿查询去搜索，召回 topK 条结果
3. **呈现 / 注入（Generation）**：传统搜索把结果展示给用户；RAG 把结果注入 LLM 上下文继续生成

区别只在第三步——"展示给用户看"变成了"塞给模型用"。

![传统搜索 vs RAG 三步流程对比](./ai-mem-rag/1.png)

搞清楚这个本质，一个常见误区就不攻自破：**RAG ≠ 向量数据库。** grep 是 RAG，SQL 全文索引是 RAG，知识图谱也是 RAG——只要符合"索引 → 检索 → 注入"这三步，都是 RAG。向量搜索只是检索手段之一，不是 RAG 的定义。
