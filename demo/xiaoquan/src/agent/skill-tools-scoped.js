/**
 * skill-tools-scoped.js — Role-scoped 技能加载器
 *
 * 修改自 skill-tools.js，但 skillsDir 绑定到 {workspaceRoot}/{role}/skills/，
 * 确保 manager 只看到 workspace/manager/skills/，pm 只看到 workspace/pm/skills/。
 */
...
export function loadRoleScopedSkillRegistry(workspaceRoot, role) {
  const skillsDir = path.join(workspaceRoot, role, 'skills')
  const registry = {}
...
export function createScopedSkillTools(registry, {sessionId, historyAll = [], role} = {}) {
  return {
    list_skills: tool({
      description: `列出 ${role || '当前角色'} 可用的所有 Skill（专项能力）及其简要描述`,
      parameters: z.object({}),
      execute: async () => {
        const list = Object.values(registry).map(s => ({name: s.name, type: s.type, description: s.description}))
        return JSON.stringify(list, null, 2)
      },
    }),

    get_skill: tool({
      description: '获取指定 Skill 的详细行为指令。执行相关任务前先调用此工具。',
      parameters: z.object({
